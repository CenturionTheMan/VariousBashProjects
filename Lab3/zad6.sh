...
#! Nie działa - dlaczego?
echo $(find linux-5.11.13/ -type f  -size -1k -perm -u=w | wc -l) # find 'f' - file, 'size' - file size, '-1k' - file size less than 1k, 'perm' - file permissions, '-u=w' - file is writable by owner